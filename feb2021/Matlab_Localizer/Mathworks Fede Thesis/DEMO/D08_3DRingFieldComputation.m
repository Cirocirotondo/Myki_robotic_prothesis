...
MM = size(MagPos,2); 
Rin = 0.001*ones(MM,1); % internal radius of the magnets
Rout = 0.002*ones(MM,1); % external radius of the magnets
...
tic
Readings = GenerateRingReadings(MagPos',SensorPosMatrix,...
                                      M,Rin,Rout,L);
toc
...
for i = 1:MM
    hold on
    drawRingMagnet(L(i),Rin(i),Rout(i),MagPos(:,i),'texture','axial')
    hold on
