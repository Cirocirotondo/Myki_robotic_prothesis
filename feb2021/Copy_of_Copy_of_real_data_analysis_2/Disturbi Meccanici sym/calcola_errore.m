function [pos_err, ang_err] = calcola_errore(x0, y, Nb_MM)
...
%% SC_POSITION
pos_err = zeros(1,Nb_MM);
for i = 1:Nb_MM
    pos_err(1,i) = norm(x0(i,1:3) - y(i,1:3));
end

%% SC_ANGULAR_ERROR
ang_err = zeros(1,Nb_MM);
for i = 1:Nb_MM
    %norm_x0 = norm(x0(i,4:6))      &righe di debug
    %norm_y = norm(y(i,4:6))
    %prod_scalare = dot(x0(i,4:6)/norm(x0(i, 4:6)), y(i, 4:6)/norm(y(i, 4:6)) )
    
    ang_err(i) = acos(abs( dot(x0(i, 4:6)/norm(x0(i, 4:6)), y(i, 4:6)/norm(y(i, 4:6))) ))*180/pi;
end
