...
%     x6Forced = cell(1,nPos);         %i magneti x6Corrected vengono forzati lungo la traiettoria. Così la posizione di partenza successiva si troverà già lungo la traiettoria
    ...
    norm_not_modified = zeros(nPos,nMag);
...
%% CALCOLO POSIZIONE SULLA TRAIETTORIA
    ...
    norm_not_modified(k,:) = calculate_norm(x6{k},trajectories_detected);
 
% %% FORZO I MAGNETI LUNGO LE TRAIETTORIE
% %in questo modo, alla successiva iterazione, l'ottimizzazione parte da un
% %punto che già si trova sulla traiettoria
% 
%     if everyone_in_trajectory == 0
%             for i = 1:nMag
%                 t = pos_along_trajectories(k,i);
%                 if t > 1
%                     t = 1;
%                 elseif t < 0
%                     t = 0;
%                 end
%                 x6SaveBeforeMoving = x6Corrected{k};
%                 x6Corrected{k}(i,1:3) = trajectories_detected(i,[1 3 5])*(1-t) + trajectories_detected(i,[2 4 6])*(t);
%             end
%     else
%         x6SaveBeforeMoving = zeros(size(x6Corrected{k}));
%     end
...
% text(R, costo, labels, 'FontSize',8 )
    ...
    pos_modified_mm = pos_along_trajectories;
    
    for i = 1:nMag
        pos_modified_mm(:,i) = pos_modified_mm(:,i) * len_trajectories(i);
        stepper_position(:,i) = stepper_positions(:,i) * len_trajectories(i);
    end
    
%     y1 = smoothdata(pos_modified_mm(:,1),'SmoothingFactor',0.1);
%     y2 = smoothdata(pos_modified_mm(:,2),'SmoothingFactor',0.1);
%     y3 = smoothdata(pos_modified_mm(:,3),'SmoothingFactor',0.1);

    y1 = smoothdata(pos_along_trajectories(:,1),'SmoothingFactor',0.1);
    y2 = smoothdata(pos_along_trajectories(:,2),'SmoothingFactor',0.1);
    y3 = smoothdata(pos_along_trajectories(:,3),'SmoothingFactor',0.1);
    ...
    for i = 1:nMag
        ...
        subplot(nMag,1, i);
        if i == 1
%             plot(1:nPos, pos_along_trajectories(:,i),'m',1:nPos, y1,'g',1:nPos, pos_along_trajectories_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,1),'k');
%               plot(1:nPos, pos_along_trajectories(:,i),'m',1:nPos, y1,'g',1:nPos, norm_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,1),'k');
            plot(1:nPos, y1,'g',1:nPos, pos_along_trajectories_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,1),'k');
%             plot(1:nPos, y1,'g', 1:nPos, stepper_positions(1+80:nPos+80,1),'k');
%               plot(1:nPos, pos_along_trajectories(:,i),'m',1:nPos, pos_along_trajectories_not_modified(:,i), 'c', 1:nPos, stepper_positions(1:nPos,1),'k');
        elseif i == 2
%             plot(1:nPos, pos_along_trajectories(:,i),'m',1:nPos, y2,'g',1:nPos, pos_along_trajectories_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,3),'k');
%                 plot(1:nPos, pos_along_trajectories(:,i),'m',1:nPos, y1*100,'g',1:nPos, norm_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,1),'k');
             plot(1:nPos, y2,'g',1:nPos, pos_along_trajectories_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+82:nPos+82,3),'k');
%              plot(1:nPos, y2,'g', 1:nPos+80, stepper_positions(1+5:nPos+85,1),'k');
...
%             plot(1:nPos, pos_along_trajectories(:,i),'m',1:nPos, y3,'g',1:nPos, pos_along_trajectories_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,2),'k');
%                 plot(1:nPos, pos_along_trajectories(:,i),'m',1:nPos, y1*100,'g',1:nPos, norm_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,1),'k');
            plot(1:nPos, y3,'g',1:nPos, pos_along_trajectories_not_modified(:,i), 'c', 1:nPos, stepper_positions(1+90:nPos+90,2),'k');
%             plot(1:nPos, y3,'g', 1:nPos, stepper_positions(1+40:nPos+40,1),'k');
