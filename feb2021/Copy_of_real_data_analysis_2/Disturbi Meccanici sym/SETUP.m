...
data = load('Data_0305_mockup\data1_20.txt');     
...
%% Orientazione "media"
% Orientazione media del magnete
% Questa parte è una gran cagata, dal momento che le evidenze sperimentali
% mostrano che il magnete cambia orientazione durante la contrazione del
% muscolo

    orientation_mean = zeros(nMag,3);
    for k = 1:nPos
        orientation_mean = orientation_mean + x6{k}(:,4:6);
    end
    orientation_mean = orientation_mean /nPos;
    
    for k = 1:nMag
        orientation_mean(k,:) = orientation_mean(k,:) / norm(orientation_mean(k,:)); 
    end
...
for k = 1:nMag
    ...
    plot_mags([r0(k,1),r0(k,2),r0(k,3),orientation_mean(k,:)], 6, 'c', 'c', 'c')
    %     testo = string(1:nPos);
