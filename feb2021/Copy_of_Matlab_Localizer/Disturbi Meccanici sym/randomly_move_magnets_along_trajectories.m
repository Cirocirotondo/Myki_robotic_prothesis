function [x,tVector] = randomly_move_magnets_along_trajectories(trajectories, nMag, t_prec)
%randomly_move_magnets places magnets in a point of the trajctory
  x = rand(nMag,6);
  tVector = zeros(nMag, 1);
  for i=1:nMag
    dt = rand(1)*0.4 - 0.2;     % c'è un dt per ogni magnete. Il modulo di dt è minore di 0.2
